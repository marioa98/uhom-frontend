...
const locations = ['Cualquiera', 'Armería', 'Colima', 'Comala', 'Coquimatlán', 'Cuauhtémoc',
  'Ixtlahuacán', 'Manzanillo', 'Minatitlan', 'Tecomán', 'Villa'].map((e, i) => ({ key: `location_${i}`, value: i, text: e }))

const square_meters = [0, 500, 1000, 5000, 10000].map((e) => ({ key: `square_${e}`, value: e, text: e === 0 ? "Cualquiera" : e }))
const price = ["0", "500,000", "1,000,000", "1,500,500"].map((e) =>
 ({ key: `price_${e}`, value: parseInt(e.split(',').join('')), text: e == "0" ? "Cualquiera" : e })
)

const rooms = [0, 1, 2, 3, 4, 5, 6].map((_p, i) => ({ key: `room_${i}`, value: i, text: i === 0 ? "Cualquiera" : i }))
const bathrooms = [0, 1, 2, 3, 4, 5, 6].map((_p, i) => ({ key: `bath_${i}`, value: i, text: i === 0 ? "Cualquiera" : i }))

const CaracteristicFilter = ({ searchProperties }) => {
